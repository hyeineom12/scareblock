...
- [wheresthejump.com](https://wheresthejump.com/full-movie-list/) — 수백 편 규모, 영화별 점프 스케어 개수·평점. 활성
...
라벨 기준(무엇을 점프 스케어 1건으로 셀 것인가) 정의용으로만 쓴다.
...
| **E3** | **카테고리별 성능** (점프 스케어 vs 지속 상태형) | **"lookahead로 분류해야 한다"의 직접 증거.** 지속 상태형은 지연 0에서도 잡히고 점프 스케어는 못 잡히면 분류가 증명된다 | 멀티카테고리 라벨 | 05 |
