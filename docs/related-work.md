...
- 조사일 — 2026.09.08 · **재조사 2026.09.13** (§3.3 타임스탬프 DB — 사이트 원문을 브라우저로 확인)
- 담당 — 개발 B (이수현)
...
"갑툭튀"는 구어다. 국내 학술 논문은 **"점프 스케어"** 를 그대로 음차해 쓰고 있으므로 그 표기를 따른다.
자체 조어를 만들면 "왜 이 말을 쓰나"가 새 질문이 되므로 통용 표기를 우선한다.

| 맥락 | 쓸 말 | 근거 |
|---|---|---|
| **현상 지칭 (주 용어)** | **점프 스케어**(jump scare) | 국내 학술 논문에서 통용 |
| 사용자에게 일어나는 것 | **놀람 반사**(startle reflex) · **청각 놀람 반응**(ASR) | 국내 심리학 표준어 |
| 계산 문제 정의 | **돌발 시청각 사건 탐지**(sudden-onset audiovisual event detection) | 본 연구가 정의 |
...
이 구분이 서면 [E5 사용자 실험](#62-우리가-실제로-실험할-수-있는-것)이 만족도 조사가 아니라
**"자극을 차단해 반응이 줄었는가"** 를 묻는 인과 실험으로 위치가 잡힌다.
...
### 1.3 점프 스케어 오디오 특징 — affective computing에 선례 있음
...
- [공포 영화에 쓰인 청각적 장치들이 공포 유발 및 내러티브 전달에 미치는 영향에 대한 연구](https://scienceon.kisti.re.kr/srch/selectPORSrchArticle.do?cn=DIKO0014783827) — 국내 선행연구. 정보처리학회 논문 관련연구 절에 넣는다
...
**그 뒷단에 사람이 만든 타임스탬프 데이터베이스가 있다** — [§3.3](#33-점프-스케어-타임스탬프-데이터베이스--네-곳-규모가-크다)에
정리한 네 곳이다. 규모 수치는 갱신되는 자체 표방 값이라 §3.3에만 둔다.

**이것이 이 문제의 현재 해법이다.** 사용자는 볼 영화를 미리 검색해 타임스탬프를 확인하고,
재생 중에 시계를 본다. **수천 편 규모가 사람 손으로 정리돼 있다는 것**이 요구가 실재한다는 증거다.

**그리고 같은 자리에서 공통된 한계가 드러난다.**

| | 타임스탬프 DB · 크라우드소싱 확장 | 이 프로젝트 |
|---|---|---|
| 대상 | **등재된 작품에만** 동작 | 임의의 영상 |
| 새 콘텐츠 | 누군가 보고 손으로 등재해야 한다 | 즉시 동작 |
| 사용자 행위 | 미리 찾아보고 **직접 피한다** | **개입** — 도달 전에 가린다 |
| 판정 주체 | 사람이 보고 판정 | 신호에서 자동 판정 |

**심사에서 나올 반박이 여기다** — *"그런 DB가 이미 있지 않나"*. 답은 **"있고, 그 규모가
수요를 증명하며, 등재되지 않은 영상에는 무력하다"** 이다.
...
### 1.6 놀람 반사 — 점프 스케어의 심리생리학적 근거

점프 스케어를 호러 팬 용어가 아니라 **측정 가능한 자극 클래스**로 정의하게 해주는 계열이다.
02단계 오디오 규칙의 근거가 엔지니어링 감이 아니라 문헌이 된다.

- [(Re)Considering the jump scare in four elements](https://doi.org/10.3389/fpsyg.2025.1569394)
  (Acosta & Sears, *Frontiers in Psychology* 16:1569394, 2026) — **점프 스케어를 4요소로 정의한다.**
  1) 단순 반사가 아니라 행동 반응을 유발하는 미적 경험, 2) **다중모달**(급작스러운 시각 침입 + 불협화 청각 폭발),
...
1. **상승 시간(rise time)을 특징에 넣어야 한다.** 문헌이 말하는 것은 얼마나 큰가가 아니라
   **얼마나 빨리 커지는가**다. 01단계 특징 목록이 `RMS · 스펙트럼 플럭스 · 직전 정적 구간`이었는데
   시간축 상승 기울기(dRMS/dt)가 빠져 있었다. 추가한다
2. **직전 정적 구간 특징이 문헌으로 정당화된다.** Acosta & Sears의 3번 요소(anticipation 단계가
   놀람 크기를 키움)가 우리가 이미 넣어둔 특징을 뒷받침한다. 엔지니어링 감이 아니라 현상 정의에서
   나온 특징이 된다

**점프 스케어 자동 탐지 연구는 여전히 확인되지 않는다.** 현상을 정의한 문헌은 있으나
자동 탐지를 다룬 연구가 없다는 것이 정확한 진술이며, 정의가 있는데 아무도 자동으로 잡지 않는다는 점에서
오히려 더 강한 주장이다.
...
| 점프 스케어 시점 정보 제공 | ❌ **타임스탬프 DB 다수 존재** | [§3.3](#33-점프-스케어-타임스탬프-데이터베이스--네-곳-규모가-크다) |
| **등재되지 않은 임의 영상에서 동작** | ✅ | 확인한 DB 네 곳과 크라우드소싱 확장 계열은 모두 사전 등재가 전제 [§3.3](#33-점프-스케어-타임스탬프-데이터베이스--네-곳-규모가-크다) · [§1.4](#14-트리거-워닝-브라우저-확장--상용-제품-있음) |
...
| **점프 스케어 자동 탐지** | ✅ | 현상 정의 문헌은 있으나(Acosta & Sears 2026) 자동 탐지 연구 미확인 |
...
**쓰임** — 점프 스케어 라벨은 아니지만 Explosion·Shooting이 "정적 → 급작스러운 큰 소리" 패턴과 겹쳐
...
**쓰임** — 시간축 라벨이 붙은 공개 벤치마크로는 점프 스케어에 가장 가깝다.
fear는 지속적 공포도 포함해 점프 스케어와 정확히 같지 않지만, **발표된 수치와 비교할 수 있다**는 점이
03단계 논문 표를 단단하게 만든다.
...
### 3.3 점프 스케어 타임스탬프 데이터베이스 — 네 곳, 규모가 크다

**조사를 다시 하니 두 곳이 더 있었고 규모도 적힌 것보다 컸다.** 다만 이 계열의 자체 표방 숫자는
**「등재 편수」·「색인 카탈로그 수」·「타임스탬프 건수」가 섞여 있고 서로 한두 자릿수씩 다르다.** 인용할 때는
사이트 원문의 단어를 그대로 옮기고 확인일을 붙인다.

| 사이트 | 규모 | 상태 |
|---|---|---|
| [NotScare](https://notscare.me/) | 원문 `9,200+ Movies · 140+ Series · 11,500+ Jumpscares` — **편수 9,200 이상, 타임스탬프 11,500건 이상.** 강도 등급 5단계(「No Jumpscares」 포함) | 활성 (2026.09.13 확인) |
| [WhenJumpScare](https://www.whenjumpscare.com/) | 원문 `424,776 films indexed · 9,506 time codes logged` (전날 424,739 — 매일 갱신) — **카탈로그 색인 수와 타임코드 건수가 다르다** | 활성 (2026.09.13 확인) |
| [Where's The Scares](https://wheresthescares.com/) | 788개 타임스탬프 / 영화 152편 · TV 4시즌 · 게임 3종. "수동 검증" 표방 | 활성 |
| [Where's The Jump](https://wheresthejump.com/full-movie-list/) | 수백 편, 영화별 개수·평점 | **2023년작 이후 등재가 없다** — 목록 열람으로 본 관찰이고 사이트 공지는 아니다 |

**제약** — API·일괄 다운로드가 없어 스크래핑이 필요하고 ToS 확인이 선행되어야 한다.
그리고 결정적으로 **전부 영화·시리즈 기반**이라 [범위 선언](../README.md#범위-선언)의
DRM 제외에 걸린다. 라벨 기준(무엇을 점프 스케어 1건으로 셀 것인가) 정의용으로만 쓴다.

> **읽는 법** — NotScare는 편수(9,200)에 비해 타임스탬프(11,500)가 적다. 등재작 상당수가 「점프 스케어
> 없음」 등급이라는 뜻이고, 그래서 **「9,200편의 타임스탬프가 있다」로 쓰면 틀린다.** 점프 스케어가 원래
> 희소하다는 방증이기도 하다 — 우리 라벨링에서 시험 클립 4개 중 3개가 0건이었던 것과 같은 방향이다.

> **다만 이것을 「데이터 후보」로만 보면 절반을 놓친다.** 사람이 손으로 수천 편 규모를 정리해
> 둔다는 사실 자체가 **수요의 증거**이고, 동시에 그 방식의 한계를 보여준다 —
> 차별점 쪽은 [§1.4](#14-트리거-워닝-브라우저-확장--상용-제품-있음)에 적었다.
...
1. **라벨 기준 정의** — 무엇을 1건으로 셀지, 구간을 어디까지 잡을지 (NotScare · Where's The Scares의 강도·타임스탬프 관행, LIRIS-ACCEDE)
...
- **NotScare 스크래핑의 ToS 적법성** 미확인(봇 접근 시 403 — 자동 수집을 막고 있다). 라벨 기준 참고용이라 소량 수동 열람으로
  대체 가능한지 먼저 판단한다
...
| # | 실험 | 무엇을 보이나 | 필요한 것 | 단계 |
|---|---|---|---|---|
| **E1** | **지연 시간별 성능 곡선** (0 · 0.5 · 1 · 2 · 3 · 5초) | **결과 그 자체.** "점프 스케어를 잡으려면 몇 초가 필요한가"에 대한 수치이며, 0초에서 무너지는 것으로 지연의 필요성이 증명된다 | 평가셋 + 규칙 탐지기 | **09.16 논문** |
...
| E5 | 사용자 실험 — 원본 vs 필터 놀람 자가보고. **피험자 내 설계**(같은 사람이 둘 다 시청, 절반씩 순서 교차) | **개입이 실제로 효과가 있는가.** 기존 계열에 없는 유일한 산출 | 15~20명 | 05 |
